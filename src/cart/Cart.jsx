...
const Cart = ({ user }) => {
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            try {
                ...
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getCart/${userEmail}`);
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            try {
                ...
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getAllProductsCartData/${userEmail}`);
                setCartItems(response.data);
            } catch (err) {
                // setError(err.message);
            } finally {
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            try {
                const userEmail = JSON.parse(localStorage.getItem('user'))?.email; // Replace with actual email logic or context
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getThisMonthBestProductsCartData/${userEmail}`);
                setCartItems(response.data);
            } catch (err) {
                // setError(err.message);
            } finally {
    ...
    const handleDelete = async (productId) => {
        try {
            ...
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/deleteflashCartItem/${userEmail}/${productId}`);
    ...
    const handleClearCart = async () => {
        try {
            const userEmail = user.email;
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/clearCart/${userEmail}`);
    ...
    const handleUpdateCart = async () => {
        try {
            ...
            const updateRequests = cartItems.map(item => {
                return axios.put(`${import.meta.env.VITE_API_URL}/api/updateCart`, {
                    email: userEmail,
        ...
        try {
            ...
            const AddDataIntoCheckoutRequests = cartItems.map(item => {
                return axios.post(`${import.meta.env.VITE_API_URL}/api/Checkout`, {
                    email: userEmail,
                    productId: item.productId,
                    quantity: item.quantity || 1, // Send the updated quantity
                    size: item.size,// Ensure size is a single value
