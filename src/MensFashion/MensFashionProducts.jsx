...
const MensFashionproducts = ({ MensFashion, user }) => {
    const [isHovered, setIsHovered] = useState(false);
    const { name, image, offerPrice, actualPrice, discount, rating, discription, _id } = MensFashion;


    const navigate = useNavigate();
    const handleClick = () => {
        navigate(`/MensFashionproductsDetails/${_id}`); // Redirect to product details page
    };
    ...
    const AddtoCart = async () => {
        try {
            ...
            await axios.post('http://localhost:3000/api/addToCartMensFashionProducts', payload, {
    ...
    return (
        ...
        >
            <div className="w-[250px] h-[300px] shadow-md  bg-white  rounded-md border-gray-300 relative flex justify-center items-center overflow-hidden">
                ...
                <button className="absolute top-0 right-10 text-white py-2" onClick={AddtoCart}>
...
export default MensFashionproducts;
