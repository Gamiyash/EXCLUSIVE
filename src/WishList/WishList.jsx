...
const WishList = ({ product, user }) => {
    ...
    useEffect(() => {
        const fetchWishlistData = async () => {
            try {
                ...
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getToWishList/${userEmail}`);
    ...
    useEffect(() => {
        const fetchWishlistData = async () => {
            try {
                const userEmail = JSON.parse(localStorage.getItem('user'))?.email; // Replace with actual email logic or context
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getToFlashWishList/${userEmail}`);
                setWishlistitems(response.data);
                // console.log("data is",Wishlistitems)
            } catch (err) {
                setError(err.message);
    ...
    useEffect(() => {
        const fetchWishlistData = async () => {
            try {
                const userEmail = JSON.parse(localStorage.getItem('user'))?.email; // Replace with actual email logic or context
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/getToThismonthWishList/${userEmail}`);
                setWishlistitems(response.data);
                // console.log("data is",Wishlistitems)
            } catch (err) {
                setError(err.message);
    ...
    const AddtoCart = async () => {
        try {
            ...
            const AddDataOfFlashsaleProductsIntoCartRequests = Wishlistitems.map(item => {
                return axios.post(`${import.meta.env.VITE_API_URL}/api/addToCart`, {
                    email: usermail,
            ...
            const AddDataOfAllProductsIntoCartRequests = Wishlistitems.map(item => {
                return axios.post(`${import.meta.env.VITE_API_URL}/api/addToCartallproduct`, {
                    email: usermail,
                    productId: item.productId,
                    quantity: item.quantity, // Send the updated quantity
                    size: item.size,// Ensure size is a single value
            ...
            const AddDataOfThisMonthProductsIntoCartRequests = Wishlistitems.map(item => {
                return axios.post(`${import.meta.env.VITE_API_URL}/api/addToCartThismonthBestProducts`, {
                    email: usermail,
                    productId: item.productId,
                    quantity: item.quantity, // Send the updated quantity
                    size: item.size,// Ensure size is a single value
    ...
    const handleDelete = async (productId) => {
        try {
            ...
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/deleteWishlistItem/${userEmail}/${productId}`);
